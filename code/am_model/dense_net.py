...
class DenseNet:
    ...
    def train_all_epochs(self, train_params):
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if train_params.get('validation_set', False):
                print("Validation...")
                _, loss, acc = self.test(self.data_provider.validation, batch_size)
                if self.should_save_logs:
    ...
    def test(self, data, batch_size):
        ...
        total_loss = []
        total_accuracy = []
        remains = num_examples

        for i in range(int(np.ceil(num_examples / float(batch_size)))):
            batch = data.next_batch(min(batch_size, remains))
            feed_dict = {
                self.images: batch[0],
                self.labels: batch[1],
            ...
            fetches = [self.prediction, self.cross_entropy, self.accuracy]
            prediction, loss, accuracy = self.sess.run(fetches, feed_dict=feed_dict)
        ...
        mean_loss = np.mean(total_loss)
        mean_accuracy = np.mean(total_accuracy)
        return predictions, mean_loss, mean_accuracy
